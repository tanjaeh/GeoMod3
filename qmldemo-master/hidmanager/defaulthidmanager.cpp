...
void DefaultHidManager::heToggleBackgroundColor() {

    static Color def_color = GMcolor::lightGrey();
    Color back_color = _gmlib->rcPair("Projection").renderer->getClearColor();
    if(back_color == GMcolor::white()) {
        _gmlib->rcPair("Projection").renderer->setClearColor(def_color);
    } else {
        if(!(back_color == def_color))
          def_color = back_color;
        _gmlib->rcPair("Projection").renderer->setClearColor(GMcolor::white());
    }
}
...
void DefaultHidManager::setupDefaultHidBindings() {
  ...
  // Rendering
    QString ha_id_render_toggle_backgroundcolor =
        registerHidAction( "Rendering",
                           "Toggle: Background color",
                           "Toggle background color",
                           this, SLOT(heToggleBackgroundColor()),
                           OGL_TRIGGER);
  ...
  registerHidMapping( ha_id_render_toggle_backgroundcolor,new KeyPressInput( Qt::Key_B, Qt::ControlModifier) );
