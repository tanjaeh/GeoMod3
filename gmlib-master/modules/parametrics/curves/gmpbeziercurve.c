 ...
 */
  ...
  /*! void PBezierCurve<T>::moveLocalCoordinatsystem(const Vector<T,3>& cm) const
   *  Move origin in local coordinate system by cm
   *
   *  Note that you have to call:
   *            sample( int m, int d ) after calling this function.
   *
   *  \param  cm      The vector to move the local origin
   */
  template <typename T>
  inline
  void PBezierCurve<T>::moveLocalCoordinatsystem(const Vector<T,3>& cm) {

      // Set origin in local coordinate system to cm
      for( int i = 0; i < _c.getDim(); i++ )
        _c[i] -= cm;
      this->translateParent( cm );
  }
   ...
   */
  template <typename T>
  void PBezierCurve<T>::prepareForSampling() {

     if(this->_parent && _coord_ch) {
       eval(_t,0);
       moveLocalCoordinatsystem(this->_p[0]);
       _coord_ch = false;
     }
std::cout << "HER ER VI!!" << std::endl;
  }
  ...
  void PBezierCurve<T>::sample( int m, int d ) {
      ...
      if(this->_local_pre_eval && this->_visu.size() < 2)
          this->_visu.resize(2);
...
/*! Point<T,3>  PBezierCurve<T>::getSamplePoint(int i, int j) const
...
Point<T,3>  PBezierCurve<T>::getSamplePoint(int i, int j) const {
  ...
  return this->_visu[1].sample_val[this->_ind_trans_pre[j]+i][0];
}



/*! Vector<T,3>& PBezierCurve<T>::getSampleDerivative(int i, int j, int d) const
 *  Return the j-derivative of sample for a given partition "j" and point nr. "i"
 *
 *  \param[in] i  The index of the point
 *  \param[in] j  The index of the partition
 *  \return       The j-derivative of sample for a given partition "j" and point "i"
 */
template <typename T>
Vector<T,3>&  PBezierCurve<T>::getSampleDerivative(int i, int j, int d) const {

  return this->_visu[1].sample_val[this->_ind_trans_pre[j]+i][d];
}



/*! int PBezierCurve<T>::getSample_d(int i, int j) const
 *  Return the number of derivatives computed at a given partition "j" and point "i"
 *
 *  \param[in] i  The index of the point
 *  \param[in] j  The index of the partition
 *  \return       The number of derivatives computed at a given partition "j" and point "i"
...
int PBezierCurve<T>::getSample_d(int i, int j) const {
  ...
  return this->_visu[1].sample_val[this->_ind_trans_pre[j]+i].getDim();
}
...
//*****************************************************
  ...
  void PBezierCurve<T>::eval( T t, int d, bool /*l*/ ) const {
    ...
    DMatrix< T > bhp;
    EvaluatorStatic<T>::evaluateBhp( bhp, getDegree(), t, 1/this->_sc );
  ...
  /*! void PBezierCurve<T>::resample( std::vector<DVector<Vector<T,n>>>& p, Sphere<T,3>& s, const std::vector<T>& t, int d) const
...
//     makeBernsteinMat(v.size(), _c.getDim()-1, this->_sc);
//     PCurve<T,3>::resample( v.sample_val, v.sur_sphere , v, d);
//   }
...
//              prepareForSampling();
  ...
  /*! void PBezierCurve<T>::localSimulate(double dt)
